...
func Query(c *gin.Context) {
	processId := c.GetString("processId")
	// start := time.Now()
	data := make(map[string]interface{})

	bindRelation := new(models.BindingRelation)
	bindRelation.OpenId = c.Query("openId")
	log.Infof("Query processId = %v,openId = %v", processId, bindRelation.OpenId)
	...
	if err != nil {
		log.Warnf("Query Query bindRelation error,processId = %v,err = %v", processId, err)
	...
	// log.Infof("Query processId = %v,result = %v,耗时 %v", processId, result, time.Since(start))
