...
func MQStart() {
	...
	if nil != err {
		log.Warnf("MQStart 初始化 Exchange:%v 出错,err = %v", bindExchange, err)
	}
	...
	if nil != err {
		log.Warnf("MQStart 初始化 Exchange:%v 出错,err = %v", programExchange, err)
	}

	bindQueue := base.GetConfig().MQs["bind"].Queue
	_, err = base.BindChannel.QueueDeclare(bindQueue, true, false, false, false, nil)
	if err != nil {
		log.Warnf("MQStart 初始化 Queue:%v 出错,err = %v", bindQueue, err)
	}
	...
	if err != nil {
		log.Warnf("MQStart 初始化 Queue:%v 出错,err = %v", programQueue, err)
	}

	bindKey := base.GetConfig().MQs["bind"].Key
	err = base.BindChannel.QueueBind(bindQueue, bindKey, bindExchange, false, nil)
	if err != nil {
		log.Warnf("MQStart 绑定 %v 到 %v 出错,err = %v", bindQueue, bindExchange, err)
	}
	...
	if err != nil {
		log.Warnf("MQStart 绑定 %v 到 %v 出错,err = %v", programQueue, programExchange, err)
	}
	log.Info("MQStart完成")
